---@overload fun(locale : localeName, line : textLine, ... : string) : string | string[]
local handler = setmetatable({
	["en-US"] = require "locale/en-US",
}, {
	__index = function (self, k)
		return assert(self["en-US"][k], string.format("%s - not a valid line!", tostring(k)))
	end,

	__call = function (self, locale, line, ...)
		locale = rawget(self, locale)
		if not (locale and locale[line]) then locale = self["en-US"] end

		assert(locale[line], "there is no line "..line)

		if ... then
			assert(type(locale[line]) == "string", line .. " does not support editable parameters")
			return locale[line]:format(...)
		else
			return locale[line]
		end
	end
})

return handler