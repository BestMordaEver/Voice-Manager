...
ratelimiter:on("channelName", function (channelID)
	if awaiting[channelID] then
		...
		if channel then
			local name = handleTemplate(channels[channelID].parent.template, channel.guild:getMember(channels[channelID].host), channels[channelID].position)
...
ratelimiter:on("companionName", function (companionID, channelID)
	if awaiting[companionID] then
		...
		if channel and companion then
			local name = handleTemplate(channels[channelID].parent.companionTemplate,
...
return function (member)
	...
	if parentData then
		if parentData.template and parentData.template:match("%%game%(?.-%)?%%") then
			local name = handleTemplate(parentData.template, member, channelData.position)
		...
		if companion and parentData.companionTemplate and parentData.companionTemplate:match("%%game%(?.-%)?%%") then
			local name = handleTemplate(parentData.companionTemplate, member, channelData.position):discordify()
